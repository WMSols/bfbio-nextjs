...
export default function NewsroomListClient() {
  ...
  return (
    <>
      <HeroSection
        title="Newsroom"
        backgroundImage="/images/investors/newsroom/hero.webp"
      ...
      <section className="py-16 md:py-24 bg-background">
        <div className="container mx-auto px-4 max-w-8xl">
          ...
          <div className="text-center mb-16 md:mb-28">
            <h2 className="text-4xl md:text-5xl   text-black mb-12">
              What's New at BF Biosciences?
            </h2>
            <p className="text-black text-lg md:text-[24px] font-light max-w-5xl  mx-auto">
            Stay updated with the latest news and developments from BF Biosciences Limited
            </p>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12 px-6 md:px-8 ">
                {items.map((article) => {
                  ...
                  return (
                    ...
                    >
                      {/* Image Container */}
                      <Link href={`/investors/newsroom/${article.slug}`} className="block relative w-full aspect-[4/3] mb-6 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden bg-muted">
                        {imageUrl ? (
                          <Image
                            src={imageUrl}
                            alt={article.title}
                      ...
                      <div className="flex flex-col flex-1 px-2">
                        <Link href={`/investors/newsroom/${article.slug}`} className="hover:text-primary transition-colors">
                        ...
                        <div className="flex items-center justify-between mt-auto pt-2">
                          <Link 
                            href={`/investors/newsroom/${article.slug}`}
                            className="text-brand text-sm font-medium underline underline-offset-4 hover:text-[#cd5caf] transition-colors"
                          >
                            Read full article
                          </Link>
